...
engine::run_update() {
    ...
    # Leave the live dashboard and return to normal terminal history.
    if $live_ui; then
        printf '\e[?25h'
    ...
    # Check for failed modules before post-install logins so login dependency
    # filters can still decide which prompts are safe to offer.
