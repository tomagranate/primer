#!/usr/bin/env bats
# modules/google-chrome/tests.bats
...
run_google_chrome_module() {
    local code="$1"
    run zsh -c "
        export PRIMER_DIR='${PRIMER_DIR}'
        export DRY_RUN='${DRY_RUN:-false}'
        export MOD_DIR='${PRIMER_DIR}/modules/google-chrome'
        export MOD_NAME='google-chrome'
...
@test "google-chrome: RPM dry-run uses Google's native package" {
    export DRY_RUN=true

    run_google_chrome_module "mod_update"

    assert_success
    assert_output --partial "google-chrome-stable_current_x86_64.rpm"
    assert_output --partial "sudo dnf5 -y --color=never install <package>"
}

@test "google-chrome: installs the native RPM" {
...
EOF
    ...
    assert_success
    run grep -F "google-chrome-stable_current_x86_64.rpm" "$MOCK_LOG"
    assert_success
}

@test "google-chrome: installs the native Debian package" {
...
EOF
    ...
    assert_success
    run grep -F "google-chrome-stable_current_amd64.deb" "$MOCK_LOG"
    assert_success
    run grep -F "apt-get install -y" "$MOCK_LOG"
    assert_success
}

@test "google-chrome: skips an installed browser" {
    echo installed > "$MOCK_INSTALLED"

    run_google_chrome_module "mod_update"

    assert_success
    run grep "$(printf 'skipped\tgoogle-chrome-stable\talready installed')" "$MOD_ITEMS_FILE"
    assert_success
